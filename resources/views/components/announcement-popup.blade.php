...
<div x-data="{ open: true }" x-show="open" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <!-- Backdrop -->
        <div x-show="open" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="fixed inset-0 bg-slate-900/75 backdrop-blur-sm transition-opacity" @click="closePopup({{ $popupAnnouncement->id }})"></div>

        <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>

        <!-- Modal Box -->
        <div x-show="open" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" class="inline-block align-bottom bg-white dark:bg-slate-900 rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-slate-100 dark:border-slate-800">
            ...
            <div class="p-6 sm:p-8 space-y-4">
                <div class="flex justify-between items-start gap-4">
                    <div>
                        ...
                        <h3 class="text-xl font-black text-slate-900 dark:text-white mt-2" id="modal-title">
                            {{ $popupAnnouncement->title }}
                        </h3>
                    </div>
                    <button type="button" @click="closePopup({{ $popupAnnouncement->id }})" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
                ...
                <div class="text-sm text-slate-600 dark:text-slate-400 leading-relaxed prose dark:prose-invert max-h-60 overflow-y-auto">
                    {!! nl2br(e($popupAnnouncement->content)) !!}
                </div>

                @if($popupAnnouncement->attachments->count() > 0)
                    <div class="pt-2 border-t border-slate-100 dark:border-slate-800">
                        <div class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Ek Dosyalar</div>
                        <div class="space-y-1">
                            @foreach($popupAnnouncement->attachments as $att)
                                <a href="{{ asset('storage/' . $att->file_path) }}" target="_blank" class="flex items-center gap-2 p-2 bg-slate-50 dark:bg-slate-800 rounded-xl text-xs font-semibold text-blue-600 dark:text-blue-400 hover:underline">
            ...
            <div class="bg-slate-50 dark:bg-slate-800/60 px-6 py-4 flex justify-end">
