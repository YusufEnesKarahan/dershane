...
@php
    ...
    $inputClasses = $type === 'radio'
        ? 'h-4 w-4 rounded-full border-slate-300 text-primary focus:ring-primary transition-all duration-200'
        : ($type === 'switch' ? 'sr-only' : 'h-4 w-4 rounded border-slate-300 text-primary focus:ring-primary transition-all duration-200');
@endphp
...
<label class="{{ $wrapperClasses }}">
    @if ($type === 'switch')
        <div class="relative flex items-center shrink-0 cursor-pointer" x-data="{ checked: {{ $checked ? 'true' : 'false' }} }">
            <input type="checkbox" 
                   ...
                   x-model="checked"
                   {{ $attributes }}>
            <div class="w-10 h-6 bg-slate-200 rounded-full transition-colors duration-200" :class="checked ? 'bg-primary' : 'bg-slate-200'"></div>
    ...
    @if ($label || $hint)
        <div class="text-sm font-sans leading-none">
            @if ($label)
                <span class="font-medium text-slate">{{ $label }}</span>
            @endif
            @if ($hint)
                <p class="text-xs text-slate/50 mt-1 leading-normal">{{ $hint }}</p>
            @endif
        </div>
    @endif
</label>
