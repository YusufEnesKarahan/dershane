...
@section('template-content')
    ...
    <x-section bg="dark" py="20" class="relative overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-primary/20">
        <x-container class="relative z-10 text-center">
            ...
            <p class="text-slate-400 max-w-2xl mx-auto">Size en yakın şubemizi ziyaret edebilir, eğitim sistemimiz hakkında detaylı bilgi alabilirsiniz.</p>
        </x-container>
    </x-section>
        ...
        <x-container>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                @foreach ($branches as $branch)
                    <div class="group bg-white rounded-premium-2xl border border-slate-100 shadow-premium-sm overflow-hidden flex flex-col sm:flex-row transition-all duration-300 hover:shadow-premium-md hover:border-primary/20">
                        <div class="w-full sm:w-1/3 h-48 sm:h-auto shrink-0 relative overflow-hidden bg-slate-100">
                        ...
                        <div class="p-6 sm:w-2/3 flex flex-col justify-center">
                            <h3 class="text-xl font-display font-bold text-slate mb-4">{{ $branch['name'] }}</h3>
                            <div class="space-y-3 text-sm text-slate/70">
