...
@section('template-content')
    ...
    <x-section bg="dark" py="20" class="relative overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-primary/20">
        <x-container class="relative z-10 text-center">
            ...
            <p class="text-slate-400 max-w-2xl mx-auto">Kurumumuzla ilgili en güncel haberler ve duyurular.</p>
        </x-container>
    </x-section>
        ...
        <x-container>
            <div class="max-w-4xl mx-auto space-y-6">
                @forelse ($announcements as $announcement)
                    <div class="bg-white p-6 md:p-8 rounded-premium-xl border border-slate-100 shadow-premium-sm relative pl-12 md:pl-16">
                        ...
                        <div class="flex items-center text-sm font-medium text-slate/50 mb-2">
                            <span>{{ $announcement['date'] }}</span>
                        </div>
                        <h3 class="text-xl font-bold font-display text-slate mb-3">{{ $announcement['title'] }}</h3>
                        <p class="text-sm text-slate/70 leading-relaxed">{{ $announcement['content'] }}</p>
                    </div>
                @empty
                    <div class="text-center py-12 bg-slate-50 rounded-premium-xl border border-slate-100">
                        ...
                        <h3 class="text-lg font-medium text-slate mb-1">Güncel Duyuru Bulunmuyor</h3>
                        <p class="text-sm text-slate/60">Şu an için yayında olan bir duyuru bulunmamaktadır.</p>
                    </div>
                @endforelse
            </div>
        </x-container>
