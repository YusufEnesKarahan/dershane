...
@section('template-content')
    ...
    <x-section bg="dark" py="20" class="relative overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-primary/20">
        <x-container class="relative z-10 text-center">
            ...
            <p class="text-slate-400 max-w-2xl mx-auto">Seminerler, konferanslar ve öğrenci motivasyon etkinliklerimizi takip edin.</p>
        </x-container>
    </x-section>
        ...
        <x-container>
            <div class="max-w-4xl mx-auto space-y-6">
                @forelse ($events as $event)
                    <div class="flex flex-col md:flex-row bg-white rounded-premium-xl border border-slate-100 shadow-premium-sm overflow-hidden group hover:shadow-premium-md transition-shadow">
                        <!-- Date Block -->
                        ...
                        <div class="p-6 md:p-8 flex-1 flex flex-col justify-center">
                            <h3 class="text-xl font-bold font-display text-slate mb-2 group-hover:text-primary transition-colors">{{ $event['title'] }}</h3>
                            <p class="text-sm text-slate/70 mb-4">{{ $event['desc'] }}</p>
                            <div class="flex items-center text-sm font-medium text-slate/60 mt-auto">
                ...
                @empty
                    <div class="text-center py-12 bg-slate-50 rounded-premium-xl border border-slate-100">
                        <svg class="h-12 w-12 text-slate-400 mx-auto mb-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5"></path></svg>
                        <h3 class="text-lg font-medium text-slate mb-1">Yaklaşan Etkinlik Bulunmuyor</h3>
                        <p class="text-sm text-slate/60">Yeni etkinlikler eklendiğinde burada listelenecektir.</p>
                    </div>
                @endforelse
            </div>
        </x-container>
