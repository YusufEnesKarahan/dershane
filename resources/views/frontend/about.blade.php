...
@section('template-content')
    ...
    <x-section bg="dark" py="20" class="relative overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-primary/20">
        <x-container class="relative z-10 text-center">
            ...
            <p class="text-slate-400 max-w-2xl mx-auto">{{ $about['story'] }}</p>
        </x-container>
    </x-section>
    ...
    <x-section bg="white" py="16" class="border-b border-slate-100">
        <x-container>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
                <div class="p-8 bg-slate-50 rounded-premium-2xl border border-slate-100 shadow-premium-sm">
                    ...
                    <h3 class="text-xl font-display font-bold text-slate mb-3">Misyonumuz</h3>
                    <p class="text-sm text-slate/70 leading-relaxed">{{ $about['mission'] }}</p>
                </div>
                <div class="p-8 bg-slate-50 rounded-premium-2xl border border-slate-100 shadow-premium-sm">
                    <div class="h-12 w-12 bg-primary/10 text-primary flex items-center justify-center rounded-xl mb-6">
                        <svg class="h-6 w-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                    </div>
                    <h3 class="text-xl font-display font-bold text-slate mb-3">Vizyonumuz</h3>
                    <p class="text-sm text-slate/70 leading-relaxed">{{ $about['vision'] }}</p>
                </div>
            </div>
        </x-container>
    </x-section>
        ...
        <x-container>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                @foreach ($about['values'] as $value)
                    <div class="text-center p-6 bg-white rounded-premium-xl border border-slate-100 shadow-premium-sm">
                        <span class="font-semibold text-slate">{{ $value }}</span>
                    </div>
                @endforeach
            </div>
        </x-container>
    ...
    <x-section bg="white" py="16" class="border-b border-slate-100">
        <x-container>
            ...
            <div class="max-w-3xl mx-auto space-y-8 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-200 before:to-transparent">
                @foreach ($about['timeline'] as $item)
                    <div class="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                        ...
                        <div class="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-6 rounded-premium-xl bg-white border border-slate-100 shadow-premium-sm">
                            <div class="flex items-center justify-between mb-2">
                                <h4 class="font-bold text-slate">{{ $item['title'] }}</h4>
                                <span class="text-xs font-semibold text-primary px-2 py-1 bg-primary/10 rounded-full">{{ $item['year'] }}</span>
                            </div>
                            <p class="text-sm text-slate/70">{{ $item['desc'] }}</p>
                        </div>
                    </div>
                @endforeach
            </div>
