...
@section('template-content')
    ...
    <x-section bg="dark" py="20" class="relative overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-primary/20">
        <x-container class="relative z-10 text-center">
            ...
            <p class="text-slate-400 max-w-2xl mx-auto">Merak ettiğiniz soruların cevaplarını burada bulabilirsiniz.</p>
        </x-container>
    </x-section>
    ...
    <x-section bg="white" py="16">
        <x-container>
            <div class="max-w-3xl mx-auto space-y-4">
                @foreach ($faqs as $question => $answer)
                    <div class="p-6 bg-white rounded-premium-xl border border-slate-100 shadow-premium-sm group hover:border-primary/20 transition-colors">
                        <h3 class="text-lg font-bold text-slate mb-2">{{ $question }}</h3>
                        <p class="text-sm text-slate/70 leading-relaxed">{{ $answer }}</p>
                    </div>
                @endforeach
            </div>
        </x-container>
