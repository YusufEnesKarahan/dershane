...
@section('template-content')
    ...
    <!-- 1. HERO SECTION -->
    <x-section bg="dark" py="24" class="relative overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-primary/10">
        ...
        <x-container class="relative z-10 flex flex-col items-center text-center space-y-8">
            ...
            <!-- Subheadline -->
            <p class="text-sm sm:text-lg text-slate-300 font-sans max-w-2xl leading-relaxed">
                {{ $hero['subtitle'] }}
            </p>
            ...
            <div class="flex flex-wrap justify-center gap-4 pt-2">
                ...
                <x-button variant="outline" size="lg" onclick="window.location.href='/kurslar'" class="text-white border-slate-700 hover:bg-slate-800/40">Eğitim Programları</x-button>
            </div>

            <!-- Success metrics directly below CTAs -->
            <div class="pt-8 border-t border-slate-800 w-full max-w-3xl grid grid-cols-4 gap-4 text-left sm:text-center select-none text-slate-400">
                @foreach ($stats as $label => $val)
                    <div>
                        <span class="block text-xl sm:text-2xl font-bold text-white tracking-tight">{{ $val }}</span>
                        <span class="text-[10px] sm:text-xs uppercase tracking-wider text-slate-500 font-semibold">{{ $label }}</span>
                    </div>
                @endforeach
            </div>
        </x-container>
    ...
    <x-section bg="white" py="16" class="border-b border-slate-100">
        <x-container>
    ...
    <x-section bg="gray" py="16" class="border-b border-slate-100">
        <x-container>
        ...
        <x-container>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                @foreach ($stories as $story)
                    <div class="p-6 bg-white rounded-premium-xl border border-slate-100 shadow-premium-sm text-center space-y-3">
                        ...
                        <h4 class="text-base font-bold text-slate">{{ $story['student'] }}</h4>
                        <p class="text-xs text-slate/50">{{ $story['destination'] }}</p>
                    </div>
                @endforeach
            </div>
        </x-container>
        ...
        <x-container>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                @foreach ($gallery as $g)
                    <div class="group relative rounded-premium-xl overflow-hidden shadow-premium-md aspect-video bg-slate-200">
                        <div class="absolute inset-0 bg-slate-950/40 opacity-80 z-10"></div>
        ...
        <x-container>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-1 space-y-6">
                    <div class="p-6 bg-slate-50 rounded-premium-xl border border-slate-100 flex items-start gap-4">
                        ...
                        <div class="font-sans">
                            <h4 class="text-sm font-semibold text-slate">Kadıköy Merkez Şubesi</h4>
                            <p class="text-xs text-slate/60 mt-1">{{ $contact['address'] }}</p>
                        </div>
                    </div>
                    ...
                    <div class="p-6 bg-slate-50 rounded-premium-xl border border-slate-100 flex items-start gap-4">
                        ...
                        <div class="font-sans">
                            <h4 class="text-sm font-semibold text-slate">Telefon Numaralarımız</h4>
                            <p class="text-xs text-slate/60 mt-1">Sabit: {{ $contact['phone'] }}</p>
                            @if(isset($contact['socials']['whatsapp']))
                                <p class="text-xs text-slate/60">WhatsApp: {{ str_replace('https://wa.me/', '+', $contact['socials']['whatsapp']) }}</p>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="lg:col-span-2 rounded-premium-2xl overflow-hidden border border-slate-100 shadow-premium-md relative min-h-[300px] bg-slate-100 flex items-center justify-center">
                    <div class="absolute inset-0 bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:16px_16px] bg-slate-50 flex flex-col items-center justify-center p-6 text-center">
                        ...
                        <h4 class="text-sm font-semibold text-slate">İnteraktif Google Harita</h4>
                        <p class="text-xs text-slate/50 max-w-sm mt-1 leading-normal">
    ...
    <x-section bg="gray" py="24">
        <x-container>
            <x-cta title="Geleceğinizi Şansa Bırakmayın" subtitle="Sınırlı VIP butik sınıf kontenjanlarımız dolmadan yerinizi ayırtın. Online ön kaydınızı hemen tamamlayın, eğitim danışmanlarımız 24 saat içinde sizi bilgilendirsin.">
                <x-button variant="primary" size="lg" onclick="window.location.href='/on-kayit'" class="shadow-premium-xl">Hemen Ön Kayıt Ol</x-button>
                @if(isset($contact['socials']['whatsapp']))
                    <x-button variant="outline" size="lg" onclick="window.location.href='{{ $contact['socials']['whatsapp'] }}'" class="border-slate-300 hover:bg-slate-200/50">
