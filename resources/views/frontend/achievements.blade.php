...
@section('template-content')
    ...
    <x-section bg="dark" py="20" class="relative overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-primary/20">
        <x-container class="relative z-10 text-center">
            ...
            <p class="text-slate-400 max-w-2xl mx-auto">Hayallerindeki okullara yerleşerek hem kendilerini hem de bizi gururlandıran başarılı öğrencilerimiz.</p>
        </x-container>
    </x-section>
        ...
        <x-container>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                @foreach ($stories as $story)
                    <div class="p-6 bg-white rounded-premium-xl border border-slate-100 shadow-premium-sm text-center space-y-3 transition duration-300 hover:shadow-premium-md hover:-translate-y-1">
                        ...
                        <h4 class="text-lg font-bold text-slate">{{ $story['student'] }}</h4>
                        <p class="text-sm text-slate/60">{{ $story['destination'] }}</p>
                    </div>
                @endforeach
            </div>
        </x-container>
