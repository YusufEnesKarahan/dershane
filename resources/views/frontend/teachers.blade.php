...
@section('template-content')
    ...
    <x-section bg="dark" py="20" class="relative overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-primary/20">
        <x-container class="relative z-10 text-center">
            ...
            <p class="text-slate-400 max-w-2xl mx-auto">Başarının tesadüf olmadığına inanan, alanında uzman ve tecrübeli eğitim kadromuz.</p>
        </x-container>
    </x-section>
        ...
        <x-container>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                @foreach ($teachers as $teacher)
                    <div class="group bg-slate-50 rounded-premium-2xl border border-slate-100 shadow-premium-sm overflow-hidden text-center transition duration-300 hover:shadow-premium-md hover:-translate-y-1">
                        <div class="w-full aspect-square bg-slate-200 relative overflow-hidden">
                            <!-- Placeholder Image -->
                        ...
                        <div class="p-6">
                            <h3 class="text-lg font-bold font-display text-slate mb-1">{{ $teacher['name'] }}</h3>
                            <p class="text-primary font-semibold text-sm mb-3">{{ $teacher['role'] }}</p>
                            <p class="text-xs text-slate/70 leading-relaxed">{{ $teacher['bio'] }}</p>
                        </div>
                    </div>
                @endforeach
            </div>
