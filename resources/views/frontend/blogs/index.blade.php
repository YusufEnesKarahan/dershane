...
@section('template-content')
    ...
    <x-section bg="dark" py="20" class="relative overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-primary/20">
        <x-container class="relative z-10 text-center">
            ...
            <p class="text-slate-400 max-w-2xl mx-auto">Rehberlik servisimizin hazırladığı sınav stratejileri ve eğitim dünyasından güncel makaleler.</p>
        </x-container>
    </x-section>
        ...
        <x-container>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach ($blogs as $blog)
                    <article class="group bg-white rounded-premium-2xl border border-slate-100 shadow-premium-sm overflow-hidden flex flex-col transition-all duration-300 hover:shadow-premium-md hover:-translate-y-1">
                        ...
                        <div class="p-8 flex-1 flex flex-col">
                            <div class="flex items-center justify-between text-xs text-slate/50 font-medium mb-3">
                                <span>{{ explode(' | ', $blog['meta'])[0] }}</span>
                                <span class="bg-primary/5 text-primary px-2 py-1 rounded-full">{{ explode(' | ', $blog['meta'])[1] }}</span>
                            </div>
                            <h3 class="text-xl font-display font-bold text-slate mb-3 group-hover:text-primary transition-colors line-clamp-2">
                                <a href="{{ $blog['link'] }}" class="focus:outline-none">
                                    <span class="absolute inset-0" aria-hidden="true"></span>
                                    {{ $blog['title'] }}
                                </a>
                            </h3>
                            <p class="text-sm text-slate/70 leading-relaxed mb-6 line-clamp-3 flex-1">{{ $blog['snippet'] }}</p>
                            <div class="flex items-center justify-between mt-auto pt-4 border-t border-slate-100">
                                <span class="text-xs text-slate/50 font-medium">{{ $blog['read_time'] }}</span>
