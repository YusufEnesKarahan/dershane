    ...
    <div class="max-w-xl w-full bg-white dark:bg-slate-800 shadow-xl rounded-3xl overflow-hidden border border-slate-100 dark:border-slate-700">
        <div class="bg-gradient-to-r from-indigo-900 to-slate-900 p-8 text-white">
            <div class="flex justify-between items-center">
                ...
                <span class="text-xs px-2.5 py-1 bg-indigo-500/20 text-indigo-300 rounded-full border border-indigo-500/30">Super Admin & Kurum</span>
            </div>
        ...
        <form action="{{ route('install.storeAdmin') }}" method="POST" class="p-8 space-y-4">
            ...
            <div class="space-y-4">
                <h3 class="text-sm font-bold text-indigo-900 dark:text-indigo-400 border-b pb-2 border-slate-100 dark:border-slate-700">Kurum Bilgisi</h3>
                <div>
                    ...
                    <input type="text" name="branch_name" id="branch_name" required value="{{ old('branch_name', 'Merkez Şube') }}" 
                        class="w-full px-4 py-2.5 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-slate-800 dark:border-slate-700">
                </div>

                <h3 class="text-sm font-bold text-indigo-900 dark:text-indigo-400 border-b pb-2 border-slate-100 dark:border-slate-700 pt-2">Super Admin Kullanıcı Bilgileri</h3>
