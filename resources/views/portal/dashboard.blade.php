...
<div class="container mx-auto px-4 py-8">
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-slate-900">Customer Portal Dashboard</h1>
        <p class="text-slate-600 mt-2">Manage your subscription, extensions, and usage from one place.</p>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <!-- Subscription Status -->
        <div class="bg-white rounded-lg shadow p-6">
            <h3 class="text-lg font-semibold text-slate-700 mb-2">Subscription</h3>
            <p class="text-2xl font-bold text-green-600">{{ $subscription['status'] ?? 'None' }}</p>
            <p class="text-sm text-slate-500 mt-1">Plan: {{ $subscription['plan_name'] ?? 'N/A' }}</p>
        </div>

        <!-- API Keys -->
        <div class="bg-white rounded-lg shadow p-6">
            <h3 class="text-lg font-semibold text-slate-700 mb-2">API Keys</h3>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <h3 class="text-lg font-semibold text-slate-700 mb-2">Support Tickets</h3>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <h3 class="text-lg font-semibold text-slate-700 mb-2">Notifications</h3>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
            ...
            <ul class="divide-y divide-slate-200 p-4">
                ...
                @empty
                    <li class="py-3 text-sm text-slate-500">No recent activity.</li>
                @endforelse
            </ul>
        </div>
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-200">
                <h3 class="text-lg font-semibold text-slate-800">Features & Entitlements</h3>
            </div>
            <ul class="divide-y divide-slate-200 p-4">
                @forelse($entitlements_summary ?? [] as $feature => $hasAccess)
                    <li class="py-3 flex justify-between items-center">
                        <span class="text-sm text-slate-700">{{ str_replace('_', ' ', Str::title($feature)) }}</span>
                        @if($hasAccess)
                            <span class="px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">Enabled</span>
                        @else
                            <span class="px-2 py-1 text-xs font-semibold rounded-full bg-slate-100 text-slate-800">Locked</span>
                        @endif
                    </li>
                @empty
                    <li class="py-3 text-sm text-slate-500">No entitlements available.</li>
                @endforelse
            </ul>
        </div>
    </div>
