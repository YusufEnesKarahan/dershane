...
<div class="container mx-auto px-4 py-8">
    <div class="mb-8 flex justify-between items-center">
        <div>
            ...
            <p class="text-slate-600 mt-2">Manage API keys to authenticate your integrations.</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <table class="min-w-full divide-y divide-slate-200">
            ...
            <tbody class="bg-white divide-y divide-slate-200">
                ...
                <tr>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900">{{ $key->name }}</td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{{ $key->created_at->format('M d, Y') }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{{ $key->last_used_at ? $key->last_used_at->diffForHumans() : 'Never' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{{ $key->expires_at ? $key->expires_at->format('M d, Y') : 'Never' }}</td>
                ...
                @empty
                <tr>
                    <td colspan="6" class="px-6 py-4 text-center text-sm text-slate-500">No API keys found.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
