...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <h1 class="text-2xl font-bold text-slate">Öğrenci Yoklama Durumu</h1>
        
        @if($students && $students->count() > 1)
        <div class="mt-4 md:mt-0">
            <form action="{{ route('parent.attendance.index') }}" method="GET" class="flex items-center">
                <label for="student_id" class="mr-2 text-sm text-slate-600">Öğrenci Seçin:</label>
                <select name="student_id" id="student_id" onchange="this.form.submit()" class="border-slate-300 rounded-md shadow-sm focus:ring-primary focus:border-primary text-sm">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <div class="bg-white p-6 rounded-lg shadow-sm border-l-4 border-primary">
            ...
            <p class="text-2xl font-bold text-slate">{{ $summary['total'] ?? 0 }}</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-200">
            <h3 class="text-lg font-medium text-slate-900">Geçmiş Yoklamalar ({{ $selectedStudent->first_name }})</h3>
        </div>
        <table class="min-w-full divide-y divide-slate-200">
            ...
            <tbody class="bg-white divide-y divide-slate-200">
                ...
                @empty
                <tr>
                    <td colspan="3" class="px-6 py-4 text-center text-sm text-slate-500">Henüz yoklama kaydı bulunmamaktadır.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
