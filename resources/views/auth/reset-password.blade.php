    ...
    <x-section bg="gray" py="24" class="min-h-screen flex items-center justify-center">
        <div class="w-full max-w-md bg-white p-8 sm:p-12 rounded-premium-2xl border border-slate-100 shadow-premium-sm">
            <div class="text-center mb-8">
                <h1 class="text-2xl font-display font-bold text-slate">Şifrenizi Sıfırlayın</h1>
                <p class="text-slate-500 mt-2 text-sm">Lütfen yeni şifrenizi belirleyin.</p>
            </div>
            ...
            <form method="POST" action="{{ route('password.update') }}" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="email" id="email" name="email" value="{{ old('email', $request->email) }}" required readonly
                        class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 text-slate-500 outline-none">
                    @error('email')
                ...
                <div>
                    <label for="password" class="block text-sm font-medium text-slate mb-2">Yeni Şifre</label>
                    <input type="password" id="password" name="password" required autofocus
                        class="w-full px-4 py-3 rounded-xl border {{ $errors->has('password') ? 'border-red-500 focus:ring-red-500/20' : 'border-slate-200 focus:border-primary focus:ring-primary/20' }} focus:ring-2 outline-none transition-all">
                    @error('password')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label for="password_confirmation" class="block text-sm font-medium text-slate mb-2">Yeni Şifre (Tekrar)</label>
