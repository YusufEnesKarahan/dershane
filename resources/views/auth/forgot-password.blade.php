    ...
    <x-section bg="gray" py="24" class="min-h-screen flex items-center justify-center">
        <div class="w-full max-w-md bg-white p-8 sm:p-12 rounded-premium-2xl border border-slate-100 shadow-premium-sm">
            <div class="text-center mb-8">
                <h1 class="text-2xl font-display font-bold text-slate">Şifremi Unuttum</h1>
                <p class="text-slate-500 mt-2 text-sm">E-posta adresinizi girin, size şifre sıfırlama bağlantısı gönderelim.</p>
            </div>
            ...
            <form method="POST" action="{{ route('password.email') }}" class="space-y-6">
                ...
                <div class="text-center mt-4">
                    <a href="{{ route('login') }}" class="text-sm text-slate-500 hover:text-primary transition-colors">Giriş Ekranına Dön</a>
                </div>
            </form>
        </div>
    </x-section>
