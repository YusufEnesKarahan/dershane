...
@section('content')
    <div class="space-y-6">
        ...
        <!-- Üst Banner -->
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-8 rounded-3xl text-slate-900 dark:text-slate-100 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-6 border border-teal-950">
            <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-premium-sm space-y-4">
                <div class="flex items-center justify-between">
                    <span class="text-xs font-bold text-slate-400 uppercase">Toplam Doküman</span>
                    <div class="p-2 bg-teal-50 dark:bg-teal-950 text-teal-600 rounded-lg">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path></svg>
                    </div>
                </div>
                <div>
                    <div class="text-3xl font-black text-slate-900 dark:text-white font-mono">{{ $metrics['total_documents'] }}</div>
                    <p class="text-[11px] text-slate-500 mt-1">Kayıtlı aktif dijital arşiv belgesi</p>
                </div>
            </div>

            <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-premium-sm space-y-4">
                <div class="flex items-center justify-between">
                    <span class="text-xs font-bold text-slate-400 uppercase">Toplam Depolama</span>
                ...
                <div>
                    <div class="text-3xl font-black text-slate-900 dark:text-white font-mono">{{ round($metrics['total_storage_bytes'] / (1024 * 1024), 2) }} MB</div>
                    <p class="text-[11px] text-slate-500 mt-1">Kullanılan sunucu disk alanı</p>
                </div>
            </div>

            <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-premium-sm space-y-4">
                <div class="flex items-center justify-between">
                    <span class="text-xs font-bold text-slate-400 uppercase">Aktif Kategoriler</span>
                    <div class="p-2 bg-blue-50 dark:bg-blue-950 text-blue-600 rounded-lg">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg>
                    </div>
                </div>
                <div>
                    <div class="text-3xl font-black text-slate-900 dark:text-white font-mono">{{ count($metrics['category_distribution']) }}</div>
                    <p class="text-[11px] text-slate-500 mt-1">Sınıflandırılmış arşiv türü</p>
                </div>
            </div>

            <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-premium-sm space-y-4">
                <div class="flex items-center justify-between">
                    <span class="text-xs font-bold text-slate-400 uppercase">Son İşlemler</span>
                    <div class="p-2 bg-amber-50 dark:bg-amber-950 text-amber-600 rounded-lg">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                    </div>
                </div>
                <div>
                    <div class="text-3xl font-black text-slate-900 dark:text-white font-mono">{{ count($metrics['recent_logs']) }}</div>
                    <p class="text-[11px] text-slate-500 mt-1">Güncel indirme & yükleme kaydı</p>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-premium-sm space-y-4">
                <h3 class="text-sm font-bold text-slate-900 dark:text-white">Kategori Dağılımı</h3>
                ...
                <div class="space-y-4">
                    @forelse($metrics['category_distribution'] as $cat)
                        <div class="space-y-1">
                            <div class="flex justify-between items-center text-xs font-bold">
                                ...
                                <span class="text-slate-500 font-mono">{{ $cat->count }} Belge</span>
                            </div>
                    ...
                    @empty
                        <div class="text-center text-xs text-slate-400 py-6">Kategori verisi bulunmamaktadır.</div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="lg:col-span-2 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-premium-sm space-y-4">
                <h3 class="text-sm font-bold text-slate-900 dark:text-white">Son Arşiv Hareketleri (Audit Logs)</h3>
                    ...
                    @forelse($metrics['recent_logs'] as $log)
                        <div class="p-3 bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 rounded-xl flex items-center justify-between text-xs">
                            ...
                            <div class="text-right">
                                <span class="px-2 py-0.5 rounded text-[9px] font-bold uppercase {{ $log->action === 'upload' ? 'bg-green-100 text-green-700' : ($log->action === 'download' ? 'bg-blue-100 text-blue-700' : 'bg-slate-200 text-slate-700') }}">
                                    {{ $log->action }}
                                </span>
                                <div class="text-[10px] font-mono text-slate-400 mt-1">{{ $log->created_at->diffForHumans() }}</div>
                            </div>
                        </div>
                    @empty
                        <div class="text-center text-xs text-slate-400 py-6">Henüz bir arşiv kaydı bulunmamaktadır.</div>
                    @endforelse
                </div>
            </div>
