...
@section('content')
    <x-admin.crud.index-layout title="Profil Bilgilerini Güncelle" description="{{ $student->full_name }} isimli öğrencinin özlük ve iletişim bilgilerini düzenleyin.">
        ...
        <div class="bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
            ...
            <x-admin.form.layout :action="route('admin.students.update', $student->id)" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    <x-admin.form.field-group label="Öğrenci No" id="student_number" required>
                        <input type="text" name="student_number" id="student_number" required value="{{ old('student_number', $student->student_number) }}" class="w-full bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 rounded-xl shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm text-slate-900 dark:text-white font-mono">
                    </x-admin.form.field-group>

                    <x-admin.form.field-group label="TC Kimlik No (11 Hane)" id="identity_number">
                        <input type="text" name="identity_number" id="identity_number" maxlength="11" value="{{ old('identity_number', $student->identity_number) }}" class="w-full bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 rounded-xl shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm text-slate-900 dark:text-white font-mono">
                    </x-admin.form.field-group>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    <x-admin.form.field-group label="Adı" id="first_name" required>
                        <input type="text" name="first_name" id="first_name" required value="{{ old('first_name', $student->first_name) }}" class="w-full bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 rounded-xl shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm text-slate-900 dark:text-white">
                    </x-admin.form.field-group>

                    <x-admin.form.field-group label="Soyadı" id="last_name" required>
                        <input type="text" name="last_name" id="last_name" required value="{{ old('last_name', $student->last_name) }}" class="w-full bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 rounded-xl shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm text-slate-900 dark:text-white">
                    </x-admin.form.field-group>

                    <x-admin.form.field-group label="Doğum Tarihi" id="birth_date">
                        <input type="date" name="birth_date" id="birth_date" value="{{ old('birth_date', $student->birth_date ? \Carbon\Carbon::parse($student->birth_date)->format('Y-m-d') : '') }}" class="w-full bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 rounded-xl shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm text-slate-900 dark:text-white">
                    </x-admin.form.field-group>
                </div>
                ...
                <div class="p-5 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700/50 mb-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <div>
                            ...
                            <input type="text" name="guardian_name" value="{{ old('guardian_name', $student->primaryGuardian?->guardian_name) }}" class="w-full bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Yakınlık (Örn: Baba)</label>
                            <input type="text" name="guardian_relation" value="{{ old('guardian_relation', $student->primaryGuardian?->relation) }}" class="w-full bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Veli Telefon (TR)</label>
                            <input type="text" name="guardian_phone" id="guardian_phone" value="{{ old('guardian_phone', $student->primaryGuardian?->phone) }}" placeholder="+90 (5XX) XXX XX XX" oninput="formatTrPhone(this)" class="w-full bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm font-mono">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Öğrenci Telefon (TR)</label>
                            <input type="text" name="phone" id="phone" value="{{ old('phone', $student->contact?->phone) }}" placeholder="+90 (5XX) XXX XX XX" oninput="formatTrPhone(this)" class="w-full bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm font-mono">
                        </div>
                    </div>
                    <div class="mt-4">
                        <label class="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1">Adres</label>
                        <textarea name="address_text" rows="2" class="w-full bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm">{{ old('address_text', $student->address?->address_text) }}</textarea>
                    </div>
                </div>
