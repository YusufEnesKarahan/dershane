...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm space-y-6 relative overflow-hidden">
            <!-- Arka plan süslemesi -->
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
                <div>
                    <div class="flex items-center gap-2 mb-1">
                        <span class="px-2 py-0.5 rounded-md bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-[10px] font-mono font-bold border border-slate-200 dark:border-slate-700">Başvuru No: {{ $admission->admission_no }}</span>
                    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="space-y-6 lg:col-span-2">
                ...
                <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm space-y-6">
                    ...
                    <!-- Evrak Yükleme Formu -->
                    <form method="POST" action="{{ route('admin.enrollment.document.upload') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4 p-5 bg-slate-50/50 dark:bg-slate-800/20 rounded-xl border border-slate-100 dark:border-slate-800">
                        @csrf
                        <input type="hidden" name="student_admission_id" value="{{ $admission->id }}">
                        
                        <div>
                            <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1.5">Belge Türü</label>
                            <select name="document_type" required class="w-full bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 rounded-lg shadow-sm focus:ring-primary focus:border-primary text-xs text-slate-900 dark:text-white transition-colors">
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1.5">Belge Adı / Açıklama</label>
                            <input type="text" name="file_name" required placeholder="Örn: Kimlik Ön Yüz" class="w-full bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 rounded-lg shadow-sm focus:ring-primary focus:border-primary text-xs text-slate-900 dark:text-white transition-colors">
                        </div>
                    ...
                    <x-admin.table.layout>
                        <x-slot name="head">
                            <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Belge Türü</th>
                            <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Dosya Adı</th>
                            <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Onay Durumu</th>
                            <th class="px-6 py-4 text-right text-xs font-bold text-slate-500 uppercase tracking-wider">İşlem</th>
                        </x-slot>
                        <x-slot name="body">
                            @forelse($admission->documents as $doc)
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                                    <td class="px-6 py-4 text-sm font-bold text-slate-900 dark:text-white">{{ $doc->document_type }}</td>
                                    <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-300 font-mono">{{ $doc->file_name }}</td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        ...
                                        @else
                                            <span class="text-slate-400 dark:text-slate-500 text-sm font-medium">Tamamlandı</span>
                                        @endif
                                    </td>
                                </tr>
                            @empty
                ...
                <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm space-y-6">
                    ...
                    <div class="space-y-4">
                        @forelse($admission->contracts as $cnt)
                            <div class="p-5 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-100 dark:border-slate-800 space-y-4">
                                <div class="flex items-center justify-between text-xs font-bold">
                                    <span class="inline-flex items-center gap-2 px-2.5 py-1 rounded-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-sm font-mono text-slate-900 dark:text-white">
                                        <svg class="w-3.5 h-3.5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>
                                        {{ $cnt->contract_no }}
                                    </span>
                                ...
                                <div class="text-[11px] text-slate-600 dark:text-slate-300 h-32 overflow-y-auto p-4 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-700 shadow-inner">
                        ...
                        @empty
                            <div class="p-6 bg-blue-50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-800/30 flex flex-col items-center justify-center text-center space-y-4">
                                <div class="w-12 h-12 bg-blue-100 dark:bg-blue-500/20 text-blue-500 dark:text-blue-400 rounded-full flex items-center justify-center">
                                ...
                                <div>
                                    <h4 class="text-sm font-bold text-blue-900 dark:text-blue-300">Sözleşme Hazırlanmadı</h4>
                                    <p class="text-xs text-blue-700/70 dark:text-blue-400/70 mt-1 max-w-md">Kayıt evrakları tamamlandıktan sonra öğrenci için sistem üzerinden otomatik sözleşme oluşturabilirsiniz.</p>
                                </div>
                ...
                @if(!in_array($admission->status, ['enrolled', 'active_student']))
                    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl text-slate-900 dark:text-slate-100 shadow-sm space-y-6 relative overflow-hidden">
                        <!-- Arka plan deseni -->
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm space-y-4">
                    ...
                    <div class="space-y-3 text-xs">
                        <div class="flex justify-between items-center py-1 border-b border-slate-50 dark:border-slate-800/50 last:border-0">
                            <span class="text-slate-500 dark:text-slate-400 font-medium">Telefon:</span>
                            <span class="font-bold text-slate-900 dark:text-white font-mono bg-slate-50 dark:bg-slate-800 px-2 py-0.5 rounded">{{ $admission->phone }}</span>
                        </div>
                        <div class="flex justify-between items-center py-1 border-b border-slate-50 dark:border-slate-800/50 last:border-0">
                            <span class="text-slate-500 dark:text-slate-400 font-medium">T.C. No:</span>
                            <span class="font-bold text-slate-900 dark:text-white font-mono bg-slate-50 dark:bg-slate-800 px-2 py-0.5 rounded">{{ $admission->tc_no ?? 'Belirtilmedi' }}</span>
                        </div>
                        <div class="flex justify-between items-center py-1 border-b border-slate-50 dark:border-slate-800/50 last:border-0">
                            <span class="text-slate-500 dark:text-slate-400 font-medium">Veli Adı:</span>
                            <span class="font-bold text-slate-900 dark:text-white">{{ $admission->guardian_name ?? 'Belirtilmedi' }}</span>
                        </div>
                        <div class="flex justify-between items-center py-1 border-b border-slate-50 dark:border-slate-800/50 last:border-0">
                            <span class="text-slate-500 dark:text-slate-400 font-medium">Toplam Ücret:</span>
                            <span class="font-bold text-slate-900 dark:text-white font-mono">₺{{ number_format($admission->total_amount, 2) }}</span>
                        </div>
                        <div class="flex justify-between items-center py-1 border-b border-slate-50 dark:border-slate-800/50 last:border-0">
                            <span class="text-slate-500 dark:text-slate-400 font-medium">Ödenen Kapora:</span>
                ...
                <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm space-y-4">
                    ...
                    <form method="POST" action="{{ route('admin.admission.note.store', $admission->id) }}" class="space-y-3">
                        @csrf
                        <textarea name="note_text" required rows="3" placeholder="Yeni bir görüşme notu ekleyin..." class="w-full bg-slate-50/50 dark:bg-slate-800/20 border-slate-300 dark:border-slate-700 rounded-lg shadow-sm focus:ring-primary focus:border-primary text-xs text-slate-900 dark:text-white transition-colors resize-none"></textarea>
                ...
                <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm space-y-4">
                    ...
                    <div class="relative pl-3 space-y-4 before:absolute before:inset-y-2 before:left-3.5 before:w-0.5 before:bg-slate-100 dark:before:bg-slate-800">
                        @forelse($admission->statusLogs as $log)
                            <div class="relative pl-5">
                                <!-- Timeline Noktası -->
                                <div class="absolute left-[-5px] top-1.5 w-2.5 h-2.5 rounded-full bg-emerald-500 ring-4 ring-white dark:ring-slate-900"></div>
                                
                                <div class="space-y-1 bg-slate-50 dark:bg-slate-800/30 p-3 rounded-xl border border-slate-100 dark:border-slate-800">
                                    <div class="flex items-center justify-between text-[10px]">
                                        <span class="font-bold text-emerald-600 dark:text-emerald-400">{{ $log->to_status }}</span>
                                        <span class="text-slate-500 dark:text-slate-400 font-mono">{{ $log->created_at->format('d.m.Y H:i') }}</span>
                                    </div>
                                    <p class="text-[11px] text-slate-600 dark:text-slate-300 leading-relaxed">{{ $log->description }}</p>
                                </div>
                            </div>
                        @empty
                            <div class="text-center text-xs text-slate-400 dark:text-slate-500 py-6 border-2 border-dashed border-slate-100 dark:border-slate-800 rounded-xl">
