                    ...
                    <div class="p-6 flex-1">
                        <x-admin.form.layout :action="route('admin.classrooms.academic-calendar.store')" method="POST">
                            <x-admin.form.field-group label="Dönem Adı" id="name" required>
                                <input type="text" name="name" id="name" required placeholder="Örn: 2026-2027 Güz Dönemi" class="w-full bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 rounded-xl shadow-sm focus:ring-primary focus:border-primary sm:text-sm text-slate-900 dark:text-white transition-colors">
                            ...
                            <div class="overflow-x-auto">
                                <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                                    <thead class="bg-slate-50/80 dark:bg-slate-900/80 backdrop-blur-sm">
                                        <tr>
                                            <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider bg-slate-50/50 dark:bg-slate-800/30">Dönem Adı</th>
                                    ...
                                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800/50 bg-white dark:bg-slate-900">
                                        @foreach($terms as $term)
                                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors border-b border-slate-100 dark:border-slate-800/50 last:border-0 group">
                                                <td class="px-6 py-4">
                                                    <div class="text-sm font-bold text-slate-900 dark:text-white">{{ $term->name }}</div>
                                            </td>
                                            <td class="px-6 py-4">
