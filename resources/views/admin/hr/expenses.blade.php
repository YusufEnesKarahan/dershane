...
@section('content')
    <x-admin.crud.index-layout title="Personel Masraf Kayıtları" description="Personellerin iş süreçlerinde yaptığı harcamaları, fiş/makbuz yüklemelerini ve masraf onaylarını yönetin.">
        <x-slot name="actions">
            <button onclick="toggleModal('expense-modal')" class="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded-xl transition-colors shadow-lg shadow-blue-900/20 border border-blue-500/50">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                        <thead class="bg-slate-50/80 dark:bg-slate-900/80 backdrop-blur-sm">
                            <tr>
                                <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider bg-slate-50/50 dark:bg-slate-800/30">Açıklama / Başlık</th>
                                ...
                                <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider bg-slate-50/50 dark:bg-slate-800/30">Kategori</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider bg-slate-50/50 dark:bg-slate-800/30">Tutar</th>
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800/50 bg-white dark:bg-slate-900">
                            @forelse($expenses as $exp)
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors border-b border-slate-100 dark:border-slate-800/50 last:border-0 group">
                                    <td class="px-6 py-4">
                                        <div class="text-sm font-bold text-slate-900 dark:text-white">{{ $exp->title }}</div>
                                        @if($exp->receipt)
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-sm font-bold text-slate-900 dark:text-white">{{ $exp->employee->first_name }} {{ $exp->employee->last_name }}</div>
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="inline-flex items-center px-2.5 py-1 rounded bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-bold border border-slate-200 dark:border-slate-700">{{ $exp->category }}</span>
                                    </td>
                                    <td class="px-6 py-4 font-mono">
                                        <span class="text-sm font-bold text-slate-900 dark:text-white">₺{{ number_format($exp->amount, 2) }}</span>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            @else
                                                <span class="text-[10px] font-medium text-slate-400 flex items-center justify-end gap-1"><svg class="w-3 h-3 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg> İşlem Tamamlandı</span>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
        ...
        <!-- Masraf Ekleme Modal -->
        <div id="expense-modal" class="fixed inset-0 z-50 hidden bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-6 max-w-md w-full shadow-premium space-y-4">
                <div class="flex justify-between items-center">
                    <h3 class="text-sm font-bold text-slate-900 dark:text-white">Yeni Masraf Beyanı Ekle</h3>
                    <button onclick="toggleModal('expense-modal')" class="text-slate-400 hover:text-slate-600">&times;</button>
                </div>
                ...
                <form method="POST" action="{{ route('admin.expenses.store') }}" class="space-y-3 text-xs">
                    ...
                    <div class="space-y-1">
                        <label class="font-bold text-slate-600 dark:text-slate-400">Masraf Açıklaması / Başlık</label>
                        <input type="text" name="title" required placeholder="Şehir dışı ulaşım, Kırtasiye alımı vb." class="w-full p-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl">
                    </div>

                    <div class="space-y-1">
                        <label class="font-bold text-slate-600 dark:text-slate-400">Tutar</label>
                        <input type="number" name="amount" required step="0.01" class="w-full p-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl">
                    </div>

                    <div class="space-y-1">
                        <label class="font-bold text-slate-600 dark:text-slate-400">Kategori</label>
                        <select name="category" required class="w-full p-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl">
                    ...
                    <div class="space-y-1">
                        <label class="font-bold text-slate-600 dark:text-slate-400">Fiş / Makbuz No</label>
                        <input type="text" name="receipt" placeholder="F-1298379" class="w-full p-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl">
                    </div>

                    <div class="flex justify-end gap-2 pt-2">
                        <button type="button" onclick="toggleModal('expense-modal')" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 font-bold rounded-xl transition">Vazgeç</button>
                        <button type="submit" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition">Beyan Et</button>
                    </div>
                </form>
            </div>
        </div>
