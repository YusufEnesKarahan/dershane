            ...
            <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col">
                ...
                <x-admin.form.layout :action="route('admin.teachers.salary.store')" method="POST">
                    ...
                    <x-admin.form.field-group label="Baz Maaş (TL)" id="base_salary">
                        <input type="number" name="base_salary" required value="45000" class="w-full text-sm bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 focus:ring-blue-500 focus:border-blue-500 dark:text-white transition-colors">
                    </x-admin.form.field-group>

                    <x-admin.form.field-group label="Ödeme Türü" id="payment_type">
                        <select name="payment_type" class="w-full text-sm bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 focus:ring-blue-500 focus:border-blue-500 dark:text-white transition-colors">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <x-admin.form.field-group label="Ek Prim" id="bonus">
                            <input type="number" name="bonus" value="0" class="w-full text-sm bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 focus:ring-blue-500 focus:border-blue-500 dark:text-white transition-colors">
                        </x-admin.form.field-group>

                        <x-admin.form.field-group label="Kesintiler" id="deductions">
                            <input type="number" name="deductions" value="0" class="w-full text-sm bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 focus:ring-blue-500 focus:border-blue-500 dark:text-white transition-colors">
                        </x-admin.form.field-group>
                    </div>
                        ...
                        <div class="divide-y divide-slate-100 dark:divide-slate-800/50">
                            @foreach($teachers as $t)
                                <div class="px-6 py-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors group">
                                    ...
                                    <span class="inline-flex items-center px-2.5 py-1 rounded bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-sm font-bold border border-slate-200 dark:border-slate-700 font-mono">
