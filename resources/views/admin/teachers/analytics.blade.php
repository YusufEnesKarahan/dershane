    ...
    <x-admin.crud.index-layout title="Öğretmen Performans & İstatistik Analizi" description="Eğitmenin aktif atandığı sınıfları, ortalama başarı puanlarını ve yoklama takip istatistiklerini izleyin.">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
                <h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Eğitmen</h4>
                <div class="text-xl font-bold text-slate-900 dark:text-white">{{ $analytics['teacher']->user->name }}</div>
                <div class="text-xs font-medium text-slate-500 mt-1">{{ $analytics['teacher']->title }}</div>
            </div>
            <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
                <h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Aktif Atanan Sınıflar</h4>
                <div class="text-2xl font-bold text-blue-600 dark:text-blue-500">{{ $analytics['assigned_classes_count'] }} Sınıf</div>
            </div>
            <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
