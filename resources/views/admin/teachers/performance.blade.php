...
@section('content')
    <x-admin.crud.index-layout title="Eğitmen Performansı" description="Eğitmen {{ $teacher->user->name }} için performans puanlaması ve değerlendirmesi yapın.">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <x-admin.form.layout :action="route('admin.teachers.performance.store')" method="POST">
                ...
                <x-admin.form.field-group label="Değerlendirme Kriteri" id="metric_type">
                    <select name="metric_type" required class="w-full text-sm bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 focus:ring-blue-500 focus:border-blue-500 dark:text-white transition-colors">
                ...
                <x-admin.form.field-group label="Performans Skoru (0 - 100)" id="score">
                    <input type="number" name="score" required min="0" max="100" value="90" class="w-full text-sm bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 focus:ring-blue-500 focus:border-blue-500 dark:text-white transition-colors">
                </x-admin.form.field-group>

                <x-admin.form.field-group label="Değerlendirme Yorumları" id="comments">
                    <textarea name="comments" rows="4" placeholder="Değerlendirme detayları ve zümre notları..." class="w-full text-sm bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 focus:ring-blue-500 focus:border-blue-500 dark:text-white transition-colors"></textarea>
                </x-admin.form.field-group>
        ...
        <div class="lg:col-span-2 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col h-full">
            ...
            <div class="p-0 flex-1 overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
                    <thead class="bg-slate-50/80 dark:bg-slate-900/80 backdrop-blur-sm">
                        <tr>
                            <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider bg-slate-50/50 dark:bg-slate-800/30">Kriter</th>
                            <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider bg-slate-50/50 dark:bg-slate-800/30">Skor</th>
                            <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider bg-slate-50/50 dark:bg-slate-800/30">Yorum</th>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800/50 bg-white dark:bg-slate-900">
                        @forelse($logs as $log)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors group">
                                <td class="px-6 py-4">
                                    <div class="text-sm font-bold text-slate-900 dark:text-white">{{ $log->metric_type }}</div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-[11px] text-slate-600 dark:text-slate-400 line-clamp-1" title="{{ $log->comments }}">{{ $log->comments ?? 'Yorumsuz' }}</div>
                                </td>
                                <td class="px-6 py-4 font-mono">
                                    <span class="text-sm font-semibold text-slate-500 dark:text-slate-400">{{ \Carbon\Carbon::parse($log->evaluated_at)->format('d.m.Y') }}</span>
                                </td>
                            </tr>
                        @empty
                            <tr>
