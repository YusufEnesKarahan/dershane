            ...
            <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col">
                ...
                <x-admin.form.layout :action="route('admin.teachers.schedules.store')" method="POST">
                    ...
                    <x-admin.form.field-group label="Sınıf" id="classroom_id">
                        <select name="classroom_id" class="w-full text-sm bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 focus:ring-blue-500 focus:border-blue-500 dark:text-white transition-colors">
                    ...
                    <x-admin.form.field-group label="Ders / Kurs" id="course_id">
                        <select name="course_id" class="w-full text-sm bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 focus:ring-blue-500 focus:border-blue-500 dark:text-white transition-colors">
                    ...
                    <x-admin.form.field-group label="Tarih" id="date">
                        <input type="date" name="date" required value="{{ date('Y-m-d') }}" class="w-full text-sm bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 focus:ring-blue-500 focus:border-blue-500 dark:text-white transition-colors">
                    </x-admin.form.field-group>

                    <div class="grid grid-cols-2 gap-4">
                        <x-admin.form.field-group label="Başlangıç Saati" id="start_time">
                            <input type="time" name="start_time" required value="09:00" class="w-full text-sm bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 focus:ring-blue-500 focus:border-blue-500 dark:text-white transition-colors">
                        </x-admin.form.field-group>

                        <x-admin.form.field-group label="Bitiş Saati" id="end_time">
                            <input type="time" name="end_time" required value="10:00" class="w-full text-sm bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 focus:ring-blue-500 focus:border-blue-500 dark:text-white transition-colors">
                        </x-admin.form.field-group>
                    </div>
                ...
                <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex-1 overflow-hidden flex flex-col">
                    <div class="p-6 border-b border-slate-100 dark:border-slate-800/50 flex items-center justify-between">
                        ...
                        <form action="{{ route('admin.teachers.schedules.index') }}" method="GET" class="flex gap-2">
                            <select name="teacher_id" onchange="this.form.submit()" class="text-xs bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 rounded-lg p-2 dark:text-white transition-colors">
                                ...
                                <div class="px-6 py-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors group">
                                    <div>
                                        <div class="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                                            ...
                                            <span class="text-[11px] font-medium text-slate-500 dark:text-slate-400">({{ $sch->classroom ? $sch->classroom->name : 'Sınıfsız' }})</span>
                                        </div>
