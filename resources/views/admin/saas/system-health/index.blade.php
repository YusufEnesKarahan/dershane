...
@section('content')
<div class="space-y-6">
    <div class="bg-gradient-to-r from-slate-900 to-emerald-900 p-8 rounded-3xl text-white shadow-md flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        <div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <x-card>
            <h3 class="text-lg font-bold mb-4 border-b pb-2 dark:border-neutral-700">Servis Durumları</h3>
        ...
        <x-card>
            <h3 class="text-lg font-bold mb-4 border-b pb-2 dark:border-neutral-700">Teknik Detaylar</h3>
            <div class="space-y-3 text-sm">
                <div>
                ...
                <div>
                    <span class="text-xs text-gray-500 block">Storage Disk</span>
                    <span class="font-bold">{{ $metrics['storage']['disk'] }}</span>
                </div>
                <div>
                    <span class="text-xs text-gray-500 block">Database Connection</span>
                    <span class="font-bold">{{ $metrics['database']['connection'] }}</span>
                </div>
                <div>
                    <span class="text-xs text-gray-500 block">Son Başarılı Cron</span>
        ...
        <x-card>
            <h3 class="text-lg font-bold mb-4 border-b pb-2 dark:border-neutral-700">Erişilebilirlik</h3>
            <div class="space-y-3 text-sm">
                <div>
                ...
                <div>
                    <span class="text-xs text-gray-500 block">Queue Pending Jobs</span>
                    <span class="font-bold">{{ $metrics['queue']['pending_jobs'] ?? 'N/A' }}</span>
                </div>
                <div>
                    <span class="text-xs text-gray-500 block">Cache</span>
                    <span class="font-bold">{{ $metrics['cache_driver'] }}</span>
                </div>
                <div>
                    <span class="text-xs text-gray-500 block">Database Error</span>
