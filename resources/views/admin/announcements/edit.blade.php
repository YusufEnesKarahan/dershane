...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-black text-neutral-900 dark:text-white">Duyuruyu Düzenle</h1>
            <p class="text-sm text-neutral-500">Duyuru başlığını, içeriğini, dosya eklerini ve yayın durumunu güncelleyin.</p>
        </div>
    ...
    <form action="{{ route('admin.announcements.update', $announcement->id) }}" method="POST" enctype="multipart/form-data" class="bg-white dark:bg-neutral-900 p-6 sm:p-8 rounded-2xl border border-neutral-100 dark:border-neutral-800 shadow-sm space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="md:col-span-2">
                ...
                <input type="text" name="title" required value="{{ old('title', $announcement->title) }}" class="w-full bg-white dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-700 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-emerald-500">
            </div>

            <div>
                <label class="block text-xs font-bold uppercase text-neutral-600 dark:text-neutral-300 mb-2">Kategori *</label>
                <select name="category_id" required class="w-full bg-white dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-700 rounded-xl px-4 py-2.5 text-sm">
        ...
        <div>
            ...
            <input type="text" name="summary" value="{{ old('summary', $announcement->summary) }}" class="w-full bg-white dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-700 rounded-xl px-4 py-2.5 text-sm">
        </div>

        <div>
            <label class="block text-xs font-bold uppercase text-neutral-600 dark:text-neutral-300 mb-2">Duyuru Detayı & İçeriği *</label>
            <textarea name="content" rows="8" required class="w-full bg-white dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-700 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-emerald-500">{{ old('content', $announcement->content) }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div>
                ...
                <input type="datetime-local" name="publish_at" value="{{ old('publish_at', $announcement->publish_at ? $announcement->publish_at->format('Y-m-d\TH:i') : '') }}" class="w-full bg-white dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-700 rounded-xl px-4 py-2.5 text-sm">
            </div>

            <div>
                <label class="block text-xs font-bold uppercase text-neutral-600 dark:text-neutral-300 mb-2">Bitiş Tarihi (expire_at)</label>
                <input type="datetime-local" name="expire_at" value="{{ old('expire_at', $announcement->expire_at ? $announcement->expire_at->format('Y-m-d\TH:i') : '') }}" class="w-full bg-white dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-700 rounded-xl px-4 py-2.5 text-sm">
            </div>
        ...
        @if($announcement->attachments->count() > 0)
            <div class="p-4 bg-neutral-50 dark:bg-neutral-800/40 rounded-xl border border-neutral-200 dark:border-neutral-700 space-y-2">
                <h4 class="text-xs font-bold text-neutral-700 dark:text-neutral-300 uppercase tracking-wider">Mevcut Ek Dosyalar</h4>
                <div class="flex flex-wrap gap-3">
                    @foreach($announcement->attachments as $att)
                        <div class="px-3 py-1.5 bg-white dark:bg-neutral-900 rounded-lg border border-neutral-200 dark:border-neutral-700 text-xs flex items-center gap-2">
                            <i class="fas fa-paperclip text-neutral-400"></i>
                            <span class="font-medium text-neutral-800 dark:text-neutral-200">{{ $att->file_name }}</span>
                        </div>
                    @endforeach
                </div>
            </div>
        ...
        <div>
            <label class="block text-xs font-bold uppercase text-neutral-600 dark:text-neutral-300 mb-2">Yeni Dosya Ekleri Ekle</label>
