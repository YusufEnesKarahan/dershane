        ...
        <x-admin.form.layout action="{{ route('admin.scholarships.update', $scholarship->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <x-admin.form.field-group label="Burs Başlığı / Sebebi" id="title" :error="$errors->first('title')" required>
                    <input type="text" name="title" value="{{ old('title', $scholarship->title) }}" class="w-full text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 text-slate-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-primary transition-colors" required>
                </x-admin.form.field-group>

                <!-- Percentage -->
                <x-admin.form.field-group label="Burs Oranı (%)" id="percentage" :error="$errors->first('percentage')" required>
                    <input type="number" step="0.01" min="0" max="100" name="percentage" value="{{ old('percentage', $scholarship->percentage) }}" class="w-full text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 text-slate-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-primary transition-colors" required>
                </x-admin.form.field-group>
            </div>
            ...
            <div class="flex items-center p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700/50 mt-6">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $scholarship->is_active) ? 'checked' : '' }} class="h-5 w-5 text-primary border-slate-300 rounded focus:ring-primary">
                <div class="ml-3">
