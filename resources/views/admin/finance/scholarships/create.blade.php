        ...
        <x-admin.form.layout action="{{ route('admin.scholarships.store') }}" method="POST">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <x-admin.form.field-group label="Burs Başlığı / Sebebi" id="title" :error="$errors->first('title')" required>
                    <input type="text" name="title" value="{{ old('title') }}" placeholder="Örn: Başarı Bursu" class="w-full text-sm bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl px-3 py-2.5 text-neutral-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-primary transition-colors" required>
                </x-admin.form.field-group>
