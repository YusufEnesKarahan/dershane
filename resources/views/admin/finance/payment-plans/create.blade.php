        ...
        <x-admin.form.layout action="{{ route('admin.payment-plans.store') }}" method="POST">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <x-admin.form.field-group label="Taksit Sayısı" id="total_installments" :error="$errors->first('total_installments')" required>
                    <input type="number" min="1" max="24" name="total_installments" value="{{ old('total_installments') }}" class="w-full text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 text-slate-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-primary transition-colors" required>
                </x-admin.form.field-group>
                ...
                <x-admin.form.field-group label="Taksit Başlangıç Tarihi" id="start_date" :error="$errors->first('start_date')" required>
                    <input type="date" name="start_date" value="{{ old('start_date', date('Y-m-d')) }}" class="w-full text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 text-slate-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-primary transition-colors" required>
                </x-admin.form.field-group>
            </div>
