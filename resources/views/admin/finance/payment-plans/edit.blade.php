        ...
        <x-admin.form.layout action="{{ route('admin.payment-plans.update', $paymentPlan->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <x-admin.form.field-group label="Taksit Sayısı" id="total_installments" :error="$errors->first('total_installments')" required>
                    <input type="number" min="1" max="24" name="total_installments" value="{{ old('total_installments', $paymentPlan->total_installments) }}" class="w-full text-sm bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl px-3 py-2.5 text-neutral-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-primary transition-colors" required>
                </x-admin.form.field-group>

                <!-- Installment Amount -->
                <x-admin.form.field-group label="Taksit Tutarı (Aylık)" id="installment_amount" :error="$errors->first('installment_amount')" required>
                    <input type="number" step="0.01" min="0.01" name="installment_amount" value="{{ old('installment_amount', $paymentPlan->installment_amount) }}" class="w-full text-sm bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl px-3 py-2.5 text-neutral-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-primary transition-colors" required>
                </x-admin.form.field-group>

                <!-- Start Date -->
                <x-admin.form.field-group label="Taksit Başlangıç Tarihi" id="start_date" :error="$errors->first('start_date')" required>
                    <input type="date" name="start_date" value="{{ old('start_date', \Carbon\Carbon::parse($paymentPlan->start_date)->format('Y-m-d')) }}" class="w-full text-sm bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl px-3 py-2.5 text-neutral-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-primary transition-colors" required>
                </x-admin.form.field-group>
            </div>
