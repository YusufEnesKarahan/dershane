...
@section('content')
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm space-y-4">
                <div class="flex items-center justify-between">
                    <span class="text-xs font-bold text-slate-400 uppercase">Toplam Demirbaş</span>
                ...
                <div>
                    <div class="text-3xl font-black text-slate-900 dark:text-white font-mono">{{ $analytics['total_assets'] }} / {{ $analytics['active_assets'] }}</div>
                    <p class="text-[11px] text-slate-500 mt-1">Toplam / Aktif Demirbaş</p>
                </div>
            </div>

            <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm space-y-4">
                <div class="flex items-center justify-between">
                    <span class="text-xs font-bold text-slate-400 uppercase">Bakımda / Zimmette</span>
                ...
                <div>
                    <div class="text-3xl font-black text-slate-900 dark:text-white font-mono">{{ $analytics['maintenance_assets'] }} / {{ $analytics['assigned_assets'] }}</div>
                    <p class="text-[11px] text-slate-500 mt-1">Bakımdaki / Çalışan Zimmetleri</p>
                </div>
            </div>

            <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm space-y-4">
                <div class="flex items-center justify-between">
                    <span class="text-xs font-bold text-slate-400 uppercase">Stok Adedi / Kritik</span>
                    <div class="p-2 bg-emerald-50 dark:bg-emerald-950 text-emerald-600 rounded-lg">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path></svg>
                    </div>
                </div>
                <div>
                    <div class="text-3xl font-black text-slate-900 dark:text-white font-mono">{{ $analytics['total_stock'] }} / <span class="text-red-500">{{ $analytics['critical_stock'] }}</span></div>
                    <p class="text-[11px] text-slate-500 mt-1">Stok Malzemeleri / Kritik Seviyedekiler</p>
                </div>
            </div>

            <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm space-y-4">
                <div class="flex items-center justify-between">
                    <span class="text-xs font-bold text-slate-400 uppercase">Aylık Alım / Bakım</span>
                ...
                <div>
                    <div class="text-3xl font-black text-slate-900 dark:text-white font-mono">₺{{ number_format($analytics['monthly_purchase_sum'] + $analytics['maintenance_cost'], 2) }}</div>
                    <p class="text-[11px] text-slate-500 mt-1">Aylık satınalma & bakım toplam giderleri</p>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm space-y-4">
                <h3 class="text-sm font-bold text-slate-900 dark:text-white">Depo / Şube Bazlı Demirbaş Dağılımı</h3>
                
                <div class="space-y-4">
                    @forelse($analytics['location_distribution'] as $loc)
                        <div class="space-y-1">
                            <div class="flex justify-between items-center text-xs font-bold">
                                <span class="text-slate-700 dark:text-slate-300">{{ $loc['name'] }}</span>
                                <span class="text-slate-500 font-mono">{{ $loc['count'] }} Demirbaş</span>
                            </div>
                    ...
                    @empty
                        <div class="text-center text-xs text-slate-400 py-6">Lokasyon verisi bulunmamaktadır.</div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="lg:col-span-2 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm space-y-4">
                <h3 class="text-sm font-bold text-slate-900 dark:text-white">Envanter Operasyonları</h3>
