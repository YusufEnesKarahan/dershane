...
@section('content')
    <x-admin.crud.index-layout title="Mesaj & Bildirim Şablonları" description="Sistem tarafından otomatik gönderilen SMS, E-Posta veya sistem bildirim metinlerini düzenleyin.">
        <form action="{{ route('admin.notifications.templates.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-3 bg-white dark:bg-neutral-900 p-6 rounded-2xl border border-neutral-100 dark:border-neutral-800 shadow-sm mb-6">@csrf
            <input name="name" required placeholder="Şablon adı" class="w-full text-sm bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl px-3 py-2.5"><input name="slug" placeholder="şablon-slug" class="w-full text-sm bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl px-3 py-2.5"><input name="title_template" required placeholder="Başlık ({{name}} destekler)" class="w-full text-sm bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl px-3 py-2.5"><select name="channel" class="w-full text-sm bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl px-3 py-2.5"><option value="panel">Panel</option><option value="email">E-posta</option><option value="sms">SMS</option></select><textarea name="body_template" required placeholder="Mesaj gövdesi" class="md:col-span-2 w-full text-sm bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl px-3 py-2.5"></textarea><x-admin.button type="submit" variant="primary" class="w-fit">Şablon ekle</x-admin.button>
        </form>
