...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-premium-sm">
            <h1 class="text-lg font-bold text-slate-900 dark:text-white">Dershane BI & Detaylı Analiz Paneli</h1>
            <p class="text-xs text-slate-500 mt-1">Öğrenci başarısı, ders doluluk oranları ve şubeler arası başarı dağılımlarının derinlemesine analizi.</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-premium-sm space-y-4">
                <h3 class="text-sm font-bold text-slate-900 dark:text-white">Yoklama & Katılım Oranları</h3>
            ...
            <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-premium-sm space-y-4">
                <h3 class="text-sm font-bold text-slate-900 dark:text-white">Sınav Başarı Net Grafiği</h3>
                
                <div class="space-y-4">
                    <div>
                        <div class="flex items-center justify-between text-xs font-bold text-slate-600 mb-1">
                            <span>Ortalama TYT Neti (120 Soru)</span>
                            <span class="text-blue-600 font-mono">{{ $metrics['avg_tyt_net'] }} Net</span>
                        </div>
                        <div class="w-full bg-slate-100 dark:bg-slate-800 h-2 rounded-full overflow-hidden">
                            @php
                                $tytPercent = $metrics['avg_tyt_net'] > 0 ? ($metrics['avg_tyt_net'] / 120) * 100 : 0;
                            @endphp
                            <div class="bg-blue-500 h-full" style="width: {{ $tytPercent }}%"></div>
                        </div>
                    </div>
            ...
            <div class="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-premium-sm space-y-4">
                <h3 class="text-sm font-bold text-slate-900 dark:text-white">Tahsilat / Alacak Analizi</h3>
